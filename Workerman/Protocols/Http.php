...
class Http
{
    ...
    public static function decode ($recv_buffer, TcpConnection $connection)
    {
        ...
        foreach ($header_data as $content) {
            ...
            switch ($key) {
                ...
                // cookie
                case 'COOKIE':
                    parse_str(str_replace('; ', '&', $_SERVER['HTTP_COOKIE']),
                            $_COOKIE);
                    break;
                // content-type
                case 'CONTENT_TYPE':
    ...
    {
        if (PHP_SAPI != 'cli') {
            return $http_response_code ? header($content, $replace,
                    $http_response_code) : header($content, $replace);
        }
     ...
     * @return bool|void
     */
    public static function setcookie ($name, $value = '', $maxage = 0, $path = '',
            $domain = '', $secure = false, $HTTPOnly = false)
    {
        if (PHP_SAPI != 'cli') {
            return setcookie($name, $value, $maxage, $path, $domain, $secure,
                    $HTTPOnly);
        }
        return self::header(
    ...
    public static function sessionCreateId ()
    {
        mt_srand();
        return bin2hex(
                pack('d', microtime(true)) . pack('N', mt_rand(0, 2147483647)));
    }
    ...
    public static function sessionId ($id = null)
    {
        ...
        if (static::sessionStarted() && HttpCache::$instance->sessionFile) {
            return str_replace('sess_', '',
                    basename(HttpCache::$instance->sessionFile));
        }
        return '';
    }
    ...
    public static function sessionStart ()
    {
        ...
        if (! isset($_COOKIE[HttpCache::$sessionName]) ||
                ! is_file(
                        HttpCache::$sessionPath . '/sess_' .
                        $_COOKIE[HttpCache::$sessionName])) {
            // Create a unique session_id and the associated file name.
            while (true) {
                $session_id = static::sessionCreateId();
                if (! is_file(
                        $file_name = HttpCache::$sessionPath . '/sess_' .
                        $session_id))
                    break;
            }
            HttpCache::$instance->sessionFile = $file_name;
            return self::setcookie(HttpCache::$sessionName, $session_id,
        ...
        if (! HttpCache::$instance->sessionFile) {
            HttpCache::$instance->sessionFile = HttpCache::$sessionPath .
                    '/sess_' . $_COOKIE[HttpCache::$sessionName];
        }
    ...
    public static function sessionWriteClose ()
    {
        ...
        if (! empty(HttpCache::$instance->sessionStarted) && ! empty($_SESSION)) {
            ...
            if ($session_str && HttpCache::$instance->sessionFile) {
                return file_put_contents(HttpCache::$instance->sessionFile,
                        $session_str);
            }
        }
    ...
    {
        $http_body = substr($http_body, 0,
                strlen($http_body) - (strlen($http_post_boundary) + 4));
                ...
                switch ($header_key) {
                    case "content-disposition":
                        ...
                        if (preg_match('/name="(.*?)"; filename="(.*?)"$/',
                            ...
                            // Parse $_POST.
                            if (preg_match('/name="(.*?)"$/', $header_value,
                                    $match)) {
    ...
    public static function tryGcSessions ()
    {
        if (HttpCache::$sessionGcProbability <= 0 ||
                ...
                rand(1, HttpCache::$sessionGcDivisor) >
                HttpCache::$sessionGcProbability) {
            return;
        }
        ...
        foreach (glob(HttpCache::$sessionPath . '/ses*') as $file) {
            if (is_file($file) &&
                    $time_now - filemtime($file) >
                    HttpCache::$sessionGcMaxLifeTime) {
                unlink($file);
            }
        }
    }
